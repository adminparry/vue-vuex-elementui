...
import { ApiUrl, localapi, proapi } from '../config/env'
...
import * as biddingProject from './tempdata/biddingProject'
import * as afterMarketInstallation from './tempdata/afterMarketInstallation'
...
if (process.env.NODE_ENV == 'development') {
	...
	*/
	...
	// my
	var biddingProjectData = () => setpromise(biddingProject.data);
	var afterMarketInstallationData = () => setpromise(afterMarketInstallation.data);
	var homeData = () => setpromise(home.data);
	
	var getMenu = () => setpromise(menu.slider);
...
export { homeData,afterMarketInstallationData,getMenu,biddingProjectData,cityGuess,  currentcity, searchplace, msiteAdress, msiteFoodTypes,shopList, searchRestaurant, foodCategory, foodDelivery, foodActivity, shopDetails, foodMenu, getRatingList, ratingScores, ratingTags, mobileCode, accountLogin, checkExsis, sendMobile, checkout, getRemark, getAddress, getcaptchas, searchNearby, postAddAddress, placeOrders, rePostVerify, validateOrders, payRequest, getService, vipCart, getHongbaoNum, getExpired, exChangeHongbao, getUser, sendLogin, getOrderList, getOrderDetail, getAddressList, getSearchAddress, deleteAddress, signout, changePassword}
